...
// Prevent Excel formula injection — prefix any string starting with a formula trigger char
...
async function buildExcel(analysisResult) {
  ...
  t1.value     = ['Bank Statement — CRA T2125 Reconciliation', safe(bankName), safe(periodPart)].filter(Boolean).join('   ·   ');
  ...
  transactions.forEach((tx, idx) => {
    ...
    // Running balance: prev + all purchases (personal + all business cols) - payments
    const vc = ws.getCell(cellRef(C.VISA, r));
    vc.value     = { formula: `${cellRef(C.VISA, r-1)}+${cellRef(C.PERSONAL, r)}+SUM(${col(C.VEH_FUEL)}${r}:${col(C.OTHER)}${r})-${cellRef(C.PMT, r)}` };

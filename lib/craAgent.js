...
2. RESTAURANT / FOOD
   A named client is being entertained (client name in description or memo) → "advertising_client"
   Team/staff dinner (staff names, "team dinner") → "staff_meals"
   Clearly during an out-of-town overnight trip (hotel on same or adjacent dates) → "travel_meals"
   Any other restaurant, café, fast food for business → "advertising_meals"
   Personal meal (no business context at all) → "personal"
...
- Parse EVERY transaction — do not skip any line
- Dates must be YYYY-MM-DD format
- "period" start/end = earliest and latest transaction dates

SET needs_review: true when ANY of the following apply:
- Merchant name is unrecognizable, truncated, or just numbers/codes
- The transaction could reasonably be personal OR business (genuinely ambiguous)
- Amount is unusually large compared to similar transactions in the statement
- Cash advance or cash withdrawal
- Foreign currency conversion charge
- Description contains "REVERSAL", "DISPUTE", "CHARGEBACK", or "ADJUSTMENT"
- The same merchant + amount appears more than once on the same date (possible duplicate)
- Any transaction you are not confident about the categorization

When needs_review is true, always fill in "notes" explaining WHY it needs review.`;
